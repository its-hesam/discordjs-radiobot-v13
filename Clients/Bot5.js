  ...
  client.on("messageCreate", message =>{
    ...
    if(command == "radio"){
        ...
        const connection = voiceDiscord.joinVoiceChannel({
            channelId: channelToJoinbot5,
            guildId: message.guild.id,
            adapterCreator: message.guild.voiceAdapterCreator,
            selfDeaf: true,
        });
        const player = voiceDiscord.createAudioPlayer();
        const resource = voiceDiscord.createAudioResource(radio["12"]); //jazz radio
    ...
    message.reply(`📻**Radio Started - RADIO JAZZ - Bot 5**`)
    }

    //restart bot 
   if(command == `reset`){
    if (message.author.id !== `${ownerid}`) return message.reply(`:x: **You Dont Have permission to use this command!** `);
    message.reply(`**Start Restarting Bot 5**`)
    client.destroy();
    client.login(token);
    message.channel.send(`✅ **Bot 5 was successfully restarted**`)
   }
// disconnect bot 
     if(command == `dc`){
	      if(message.author.id !== ownerid && !message.member.roles.cache.has(role))return message.reply(`:x: **You Dont Have permission to use this command! , you need <@&${role}> role**`)
    connection.destroy();
    message.reply('✅ **Bot 5 was successfully Disconnected** ')
     }

})
    
   client.login(tokenBot5);
